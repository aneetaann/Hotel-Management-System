...
export class MdashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.httpc.get<Object>(`http://localhost:3000/manager/profile`).subscribe(
      (res) => {
            console.log(res)
  ...
  editRoom(someID:any){
    console.log(someID)
    this.httpc.get<any>(`http://localhost:3000/manager/profile/${someID}`).subscribe((x) => {
      this.currentRoom = x
  ...
  save(){
    ...
    this.httpc.put<any>(`http://localhost:3000/manager/profile/${this.currentRoom._id}`,obj).subscribe((x) => {
      console.log(x)
      switch(x.message){
